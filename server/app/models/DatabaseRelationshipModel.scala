...
class DatabaseRelationshipModel(db: Database)(implicit ec: ExecutionContext) {
  type Username = String
  type Relationship = String

  def getRelationships(): Future[Seq[(Username, Username, Relationship)]] = {
    db.run((for {
      relationship <- Relationships
      u1 <- Users; if u1.id === relationship.userId1
      u2 <- Users; if u2.id === relationship.userId2
    } yield (u1.name.get, u2.name.get, relationship.relation)).result)
  }
